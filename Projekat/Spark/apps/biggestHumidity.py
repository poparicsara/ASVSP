...
if __name__ == "__main__":
    ...
    humidity_df = df.groupBy("weather_city_name").agg({'weather_humidity': 'max'})\
                 .select("weather_city_name", col("max(weather_humidity)").alias("humidity"))

    query=humidity_df.writeStream.outputMode("update") \
        .foreachBatch(lambda df, epoch_id: write_result(df, epoch_id, "humidity")) \
        .start()
